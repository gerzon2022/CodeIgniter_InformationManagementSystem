...
class DashboardModel extends CI_Model
{
    ...
    {
        return $this->db->get('house_number')->result_array();
    }
    ...
    public function getlatestAnnouncement()
    {
        ...
        $this->db->where('day(NOW())-day(date) <', 1);
        $this->db->order_by('day(NOW())-day(date)', 'DESC');
        $this->db->limit(2);
    ...
    {
        $this->db->where('status', 'Active');
        $this->db->order_by('day(NOW())-day(date)', 'ASC');
        $query = $this->db->get('announcement');
        return $query->result_array();
    }
